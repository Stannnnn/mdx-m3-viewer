...
class M3Layer {
    constructor(material, index, layerReference, type, op) {
        ...
        if (layerReference) {
            ...
            let source = layer.imagePath.get().toLowerCase();
            if (source.length) {
                this.source = source;
                this.active = 1;
                ...
                let m3Texture = new texture_1.default(!!(flags & 0x4), !!(flags & 0x8));
    ...
    bind(shader, textureOverrides) {
        ...
        if (active) {
            let m3Texture = this.texture;
            let texture = textureOverrides.get(this.material.index * standardmaterial_1.STANDARD_MATERIAL_OFFSET + this.index) || m3Texture.texture;
