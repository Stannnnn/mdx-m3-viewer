    ...
    if (geosets.length > 0) {
        ...
        for (let i = 0, l = geosets.length; i < l; i++) {
            ...
            if (geoset.lod === 0 || geoset.lod === -1) {
                ...
                let vGeoset = new geoset_1.default(model, model.geosets.length, positionOffset, normalOffset, uvOffset, skinOffset, faceOffset, vertices, faces.length, geoset.faceTypeGroups[0]);
                model.geosets.push(vGeoset);
                if (batchType === HD_BATCH) {
                    model.batches.push(new batch_1.default(model.batches.length, vGeoset, model.materials[geoset.materialId], false, true));
                }
                else {
                    let isExtended = batchType === EXTENDED_BATCH;
                    for (let layer of model.materials[geoset.materialId].layers) {
                        model.batches.push(new batch_1.default(model.batches.length, vGeoset, layer, isExtended, false));
                    }
                }
                // Positions.
