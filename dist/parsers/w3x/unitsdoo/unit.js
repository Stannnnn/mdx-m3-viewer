...
class Unit {
    ...
    load(stream, version, subversion, isReforged) {
        ...
        for (let i = 0, l = stream.readInt32(); i < l; i++) {
            let set = new droppeditemset_1.default();
            set.load(stream);
        ...
        for (let i = 0, l = stream.readInt32(); i < l; i++) {
            let item = new inventoryitem_1.default();
            item.load(stream);
            this.itemsInInventory[i] = item;
        }
        for (let i = 0, l = stream.readInt32(); i < l; i++) {
            let modifiedAbility = new modifiedability_1.default();
        ...
        if (this.randomFlag === 0) {
            ...
            for (let i = 0, l = stream.readInt32(); i < l; i++) {
                let randomUnit = new randomunit_1.default();
