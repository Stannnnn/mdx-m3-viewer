...
class MpqArchive {
    ...
    load(buffer, readonly = false) {
        ...
        let headerOffset = isarchive_1.searchHeader(bytes);
        if (headerOffset === -1) {
        ...
        let hashPos = typecast_1.numberToUint32(uint32array[4] + headerOffset); // Whoever thought of MoonLight, clever!
        let blockPos = typecast_1.numberToUint32(uint32array[5] + headerOffset);
        ...
        for (let hash of this.hashTable.entries) {
            ...
            if (blockIndex < constants_1.HASH_ENTRY_DELETED) {
                let file = new file_1.default(this, hash, this.blockTable.entries[blockIndex], bytes, null);
    ...
    set(name, buffer) {
        ...
        if (buffer instanceof ArrayBuffer) {
            ...
            bytes = utf8_1.encodeUtf8(buffer);
        }
    ...
    resizeHashtable(size) {
        ...
        size = Math.max(4, math_1.powerOfTwo(size));
