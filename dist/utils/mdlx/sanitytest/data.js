...
class SanityTestData {
    ...
    addObjects(objects) {
        if (objects.length) {
            let areGeneric = objects[0] instanceof genericobject_1.default;
            for (let i = 0, l = objects.length; i < l; i++) {
                let object = objects[i];
                let name = utils_1.getObjectName(object, i);
                let node = { type: 'node', name, errors: 0, severe: 0, warnings: 0, unused: 0, nodes: [] };
                if (!areGeneric) {
                    node.uses = 0;
                }
    ...
    push(object, index) {
        ...
        if (!node) {
            let name = utils_1.getObjectName(object, index);
