...
function convertTrigger(data, trigger, callbacks) {
    let name = utils_1.ensureNameSafety(trigger.name);
    let events = [];
    let conditions = [];
    let actions = [];
    ...
    if (events.length || conditions.length || actions.length) {
        ...
        let triggerParameter = new parameter_1.default();
...
function convertFunctionCall(data, object, callbacks) {
    ...
    let ecaObject = object; // Get correct typing for the cases where the object is known to be an ECA.
    ...
    if (name === 'IfThenElse') {
        ...
        let callbackName = `Trig_${utils_1.ensureNameSafety(triggerName)}_Func${callbacks.length}`;
    ...
    return ecas.map((eca) => utils_1.createCustomScriptECA(eca));
}
